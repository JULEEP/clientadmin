...
import { API_BASE_URL, API_DOMAIN } from "../utils/config";
  ...
  const downloadCSV = () => {
    ...
    const csvRows = [
      ...
      ...filteredCertificates.map(c => {
        ...
        const displayName = c.employeeName || c.candidateName || 'N/A';
        return [
          `"${displayName}"`,
          `"${displayId}"`,
                  ...
                  currentItems.map((c, idx) => {
                    ...
                    return (
                      <tr key={c._id} className="transition border-b hover:bg-gray-50">
                        ...
                        <td className="px-2 py-2 text-sm font-medium text-center text-gray-900 whitespace-nowrap">
                          <div className="font-semibold">{c.employeeName || c.candidateName || "N/A"}</div>
                        </td>
